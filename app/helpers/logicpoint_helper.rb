module LogicpointHelper
  ...
  class LogicpointBuilder
    attr_accessor :form, :field_compares, :value_compares, :value_ins, :shows,
                  :is_blanks, :not_blanks, :enables, :uuid

    # TODO: is_blank, not_blank

    def initialize(form)
      self.form, self.field_compares, self.value_compares, self.shows = form, [], [], []
    ...
    def field_compare(f1, script_operator, f2)
    ...
    def single_fields
      (self.value_compares + self.value_ins + self.is_blanks + self.not_blanks).map {|x| x[:f1]}
    end
